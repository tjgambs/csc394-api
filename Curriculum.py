class Curriculum:
    ...
    advanced_courses = set()

    # All courses included in this curriculum
    coursesInCurriculum = set()
    ...
    def __init__(self, courseTypeDesignations, reqs):
        ...
        self.advanced_courses = courseTypeDesignations[13]

        self.gradReqs = reqs

        self.coursesInCurriculum = self.introductory_courses.union(self.foundation_courses).union(self.major_elective_courses).union \
            (self.open_elective_courses).union(self.software_systems_dev_courses).union(self.theory_courses).union \
            (self.data_science_courses).union(self.database_systems_courses).union(self.ai_courses).union \
            (self.software_engineering_courses).union(self.game_and_real_time_systems_courses).union \
            (self.human_computer_interaction_courses).union(self.capstone_courses)
